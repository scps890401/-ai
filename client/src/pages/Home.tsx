...
export default function Home() {
  ...
  const feedbackSubmit = trpc.feedback.submit.useMutation({ onSuccess: (result) => { setFeedbackMessage(""); setFeedbackContact(""); toast.success(result.notified ? "回饋已送出" : "回饋已保存", { description: result.notified ? "謝謝你的建議，我會收到通知。" : "通知服務暫時忙碌，但內容已保存。" }); }, onError: (error) => { toast.error("回饋送出失敗", { description: error.message || "請稍後再試。" }); } });
