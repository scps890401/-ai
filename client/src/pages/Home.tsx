...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const latestZip = projectResume.data?.exports?.find((item) => item.type === "zip" && item.status === "ready");
    if (latestZip?.url) setSavedZipUrl(latestZip.url);
  ...
  async function exportZip() {
    ...
    try {
      ...
      let persistedExport = false;
      try {
        const id = await ensureProjectId();
        if (id) {
          const prepared = await prepareProjectExportUpload.mutateAsync({ projectId: id, guestKey: guestKey ?? undefined, type: "zip", fileName: "line-sticker-set.zip", byteSize: file.size });
          const upload = await fetch(prepared.uploadUrl, { method: "PUT", headers: { "Content-Type": "application/zip" }, body: file });
          if (!upload.ok) throw new Error(`ZIP storage upload failed (${upload.status})`);
          const saved = await registerProjectExport.mutateAsync({
            projectId: id,
            guestKey: guestKey ?? undefined,
            type: "zip",
            storageKey: prepared.key,
            validationJson: JSON.stringify({ lineStaticSticker: true, packSize, stickerCount: generated.length, totalBytes, outputs: [...stickerBlobs.map((blob, index) => ({ file: lineOutputFileName(index + 1), bytes: blob.size })), ...extras.map((blob, index) => ({ file: extraOutputs[index]!.file, bytes: blob.size }))] }),
          });
          setSavedZipUrl(saved.url);
          persistedExport = true;
        }
      } catch (error) {
        console.warn("ZIP export persistence failed", error);
      }
      ...
      setProcessingProgress(100);
      toast.success(`已打包 ${generated.length} 張貼圖`, { description: persistedExport ? "LINE 檢查完成，ZIP 已保存到此專案並開始下載。" : "LINE 檢查完成並開始下載；這次 ZIP 未能保存到專案。" });
    } catch {
