...
export default function Home() {
  ...
  const learnedConcepts = useMemo(() => learningEnabled ? [...RABBIT_EXAMPLES.map(rabbitExampleToStickerConcept), ...(learnedIdeasQuery.data ?? []).map((idea) => ({ key: `learned:${idea.id}`, scenarioKey: `learned:${idea.id}`, scenario: "你的創作", text: idea.text, action: idea.action }))] : [], [learnedIdeasQuery.data, learningEnabled]);
  ...
  function useRabbitExample(example: RabbitExample) {
    const draft = applyRabbitExampleToComposer(imagePrompts, example);
    setMode(draft.mode);
    setPrompt(draft.prompt);
    setImagePrompts(draft.imagePrompts);
    toast.success("已帶入兔子貼圖構思", { description: `${example.text} · ${example.action}` });
  }
